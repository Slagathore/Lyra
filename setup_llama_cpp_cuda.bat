@echo off
echo =====================================================
echo Setting up environment for llama-cpp-python with CUDA
echo =====================================================
echo.
...
echo.
echo Step 2: Creating conda environment for llama-cpp...

:: Initialize command prompt to use conda
call "%MINICONDA_PATH%\Scripts\activate.bat"

:: Create conda environment if it doesn't exist
conda info --envs | findstr /C:"llama-env" >nul
if %errorlevel% neq 0 (
    echo Creating new conda environment: llama-env
    call conda create -y -n llama-env python=3.10
) else (
    echo Environment llama-env already exists
)
...
echo.
echo Step 3: Cleaning environment for fresh install...

:: First uninstall llama-cpp-python (ignore errors)
echo Uninstalling any existing llama-cpp-python installations...
pip uninstall -y llama-cpp-python 2>nul
pip uninstall -y llama-cpp-python-cuda 2>nul

:: Check if packages exist before attempting to uninstall
echo Checking existing PyTorch installations...
...
:: Check conda packages more carefully
echo Checking conda packages...
FOR %%P IN (pytorch torchvision torchaudio pytorch-cuda cudatoolkit) DO (
    conda list | findstr %%P >nul
    if !errorlevel! equ 0 (
        echo Removing conda package: %%P
        conda remove -y %%P
    )
)
...
echo.
echo Step 4: Setting up CUDA environment (11.8)...

:: Add CUDA 11.8 to PATH
set "CUDA_PATH=C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v11.8"
set "PATH=%CUDA_PATH%\bin;%PATH%"

echo Finding nvcc location...
where nvcc
if %errorlevel% neq 0 (
    echo WARNING: nvcc not found in PATH. Please make sure CUDA 11.8 is installed.
)
...
:: Verify CUDA setup
echo.
echo Checking if nvcc is visible inside the Virtual Environment...
nvcc --version
if %errorlevel% neq 0 (
    echo WARNING: Failed to find nvcc. This may affect compilation.
    echo Continuing anyway but CUDA support might not work correctly.
)

echo.
echo Step 5: Installing PyTorch 2.1.0 with CUDA 11.8 support...
echo This will ensure compatibility with llama-cpp-python...
...
echo.
echo Step 6: Installing llama-cpp-python with CUDA...
...
:: First try with pre-built wheels (faster and more reliable)
echo Trying pre-built CUDA wheels first (faster installation)...
...
if %errorlevel% neq 0 (
    echo Pre-built wheel installation failed, building from source...
...
echo     conda activate llama-env
echo.
echo Creating a shortcut script to activate the environment...
echo @echo off > llama_env.bat
echo call "%MINICONDA_PATH%\Scripts\activate.bat" >> llama_env.bat
echo call conda activate llama-env >> llama_env.bat
echo echo Llama environment activated. >> llama_env.bat
echo echo Run your llama-cpp-python scripts now. >> llama_env.bat
echo cmd /k >> llama_env.bat

echo.
echo You can now run llama_env.bat to activate the environment.
echo ======================================================
pause
